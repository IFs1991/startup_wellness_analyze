...
- POST /api/kti/visualize - 知識移転指数分析結果の可視化
- POST /api/kti/analyze-and-visualize - データ分析と可視化を一度に実行
"""
...
from analysis.KnowledgeTransferIndexCalculator import KnowledgeTransferIndexCalculator
...
class KTIAnalysisParams(BaseModel):
...
class KTIAnalysisRequest(BaseModel):
    params: KTIAnalysisParams
...
class KTIVisualizationRequest(BaseModel):
    analysis_results: Dict[str, Any] = Field(..., description="知識移転指数分析結果")
...
class KTIVisualizationResponse(BaseModel):
...
class KTIAnalysisError(APIError):
    def __init__(self, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST,
            code="KTI_ANALYSIS_ERROR",
            message=message,
            details=details
        )

class InvalidKTIDataError(ValidationFailedError):
    def __init__(self, message: str, details: Optional[Dict[str, Any]] = None):
        super().__init__(message=message, details=details)

# ヘルパー関数例
def _prepare_chart_data_from_kti(
    ...
    if not analysis_results:
        raise InvalidKTIDataError(
            message="可視化するデータが見つかりません",
            details={"reason": "知識移転指数分析結果が空です"}
        )
...
@router.post("/visualize", response_model=KTIVisualizationResponse, status_code=status.HTTP_200_OK)
async def visualize_kti_analysis(
    request: KTIVisualizationRequest,
    current_user: User = Depends(get_current_user),
    visualization_service = Depends(get_visualization_service),
    settings: Settings = Depends(get_settings)
):
    try:
        logger.info(f"知識移転指数分析の可視化リクエスト受信: タイプ={request.visualization_type}")
        chart_data = _prepare_chart_data_from_kti(request.analysis_results, request.visualization_type, request.options)
        chart_id = "dummy_chart_id"
        url = f"https://dummy.url/{chart_id}"
        return KTIVisualizationResponse(
            chart_id=chart_id,
            url=url,
            format="png",
            thumbnail_url=None,
        ...
        logger.error(f"知識移転指数可視化エラー: {str(e)}")
        raise KTIAnalysisError(message=str(e))